...
from main import main, summarize_and_notify_emails
import uvicorn
import asyncio
import aiohttp
...
async def summarize_emails_endpoint():
    print("Processing emails for summarization and notification...")
    try:
        result = await asyncio.to_thread(summarize_and_notify_emails)
        print(result)
        return result
    except Exception as e:
